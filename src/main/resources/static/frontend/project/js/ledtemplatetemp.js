...
$(function() {
	...
	$("#add_btu").click(function() {
		...
		$("#add_ggmc").textbox("setValue", "");
		$("#add_fbsj").textbox("setValue", "");
		$("#add_jzsj").textbox("setValue", "");
...
function addSaveBtuFun() {
	...
	var ggmc = $("#add_ggmc").textbox("getValue");
	var mbid = $("#add_mbid").textbox("getValue");
	var mblj = $("#add_mblj").textbox("getValue");
	var fbsj = $("#add_fbsj").textbox("getValue");
	var jzsj = $("#add_jzsj").textbox("getValue");
	// 校验ggmc，fbsj，jzsj是否填写
	if (ggmc == null || ggmc == "" || fbsj == null || fbsj == "" || jzsj == null || jzsj == "") {
		$.messager.alert("提示", "公告名称、发布时间和截止时间都不能为空!");
		return false;
	}
	// 验证数据填写是否符合规则
	if(checkSendTableData(rows)) {
		...
		// 向后台发送数据
		$.post("/saveMbTempinfo", resMap, function(data){
...
function clearAndInitFqmbpz(type, paramdData) {
	...
	if(ADDTYPE == type) {
		...
		$("#add_ggmc").textbox("setValue", rowData.ggmc);
		$("#add_fbsj").textbox("setValue", rowData.fbsj);
		$("#add_jzsj").textbox("setValue", rowData.jzsj);
...
function searchFqmb() {
	var map = {};
	map.ggmc = $("#search_ggmc").textbox("getValue");
	map.fbsj = $("#search_fbsj").textbox("getValue");
	map.jzsj = $("#search_jzsj").textbox("getValue");
	$('#datagrid').datagrid({
		url: "/selectLedTemplateTempByInfo",
		queryParams: map,
...
function searchLedList() {
	...
	$.post('/selectXfzpList', map, function(data){
		$("#select1").empty();
		$("#select2").empty();
		// 装配select的option
...
$("#add_zdsz_save_btu").bind('click', function() {
	...
	$.post('/saveFpzpToLedTempRelTemp', param, function(data){
		if (data.status == "0") {
			$.messager.alert("提示", data.message);
			return false;
		} else {
			$("#xfzp_dialog").dialog("close");
			$.messager.alert("提示", data.message);
			searchFqmb();
		}
	});
 ...
 * @returns
 */
$("#fbzp_btu").bind('click', function(){
	var rowData = $("#datagrid").datagrid("getSelected");
	var param = {};
	param.mbid = rowData.mbid;
	$.post('/fbggLedTemplateTemp', param, function(data){
		if (data.status == "0") {
			$.messager.alert("提示", data.message);
			return false;
		} else {

...
$("#btn5").bind("click", function() {
	...
	addFouce();
	//console.log($('#test1').next('span').find('input'));
...
function addFouce(){
	$('#test1').next('span').find('input').focus();
	...
	/*$(window).keydown(function(event) {//按键事件
		if (event.keyCode == 13){
			if(n < items.length){
				$('#'+items[n]).next('span').find('input').focus();
				n++;
			}else{
				//这里放保存方法
				alert("结束");
	...
	$('.testCombo').textbox('textbox').on('keydown',function (event) {
		if (event.keyCode == 13) {
			alert(11);
			if(n < items.length){
				$('#'+items[n]).next('span').find('input').focus();

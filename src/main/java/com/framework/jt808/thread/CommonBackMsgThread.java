...
public class CommonBackMsgThread implements Runnable {
	...
	public CommonBackMsgThread() {
	}

	public CommonBackMsgThread(ConcurrentLinkedQueue<PackageData> msgQueue, LedMsgProcessService msgProcessService) {
	...
	void dealQueque(PackageData packageData) {
		if(packageData != null) {
			try {
				msgProcessService.ledCommonBackDetail(packageData);
			} catch (Exception e) {

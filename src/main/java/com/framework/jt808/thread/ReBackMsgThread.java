...
public class ReBackMsgThread implements Runnable {
	...
	public ReBackMsgThread() {
	}

	public ReBackMsgThread(ConcurrentLinkedQueue<PackageData> msgQueue, LedMsgProcessService msgProcessService) {
	...
	void dealQueque(PackageData packageData) {
		if(packageData != null) {
			try {
				msgProcessService.generateMsg0901Body(packageData);
			} catch (Exception e) {

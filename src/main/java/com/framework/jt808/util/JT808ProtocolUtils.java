...
public class JT808ProtocolUtils {
	 ...
	 * @throws Exception
	 */
	public synchronized byte[] doEscape4Receive(byte[] bs, int start, int end) throws Exception {
...
//			}
	 ...
	 * @throws Exception
	 */
	public synchronized byte[] doEscape4Send(byte[] bs, int start, int end) throws Exception {
		if (start < 0 || end > bs.length)
...
//			}
	...
	public synchronized int generateMsgBodyProps(int msgLen, int enctyptionType, boolean isSubPackage, int reversed_14_15) {
	...
	public synchronized byte[] generateMsgHeader(String phone, int msgType, byte[] body, int msgBodyProps, int flowId)
			throws Exception {
		ByteArrayOutputStream baos = null;
		try {
			baos = new ByteArrayOutputStream();

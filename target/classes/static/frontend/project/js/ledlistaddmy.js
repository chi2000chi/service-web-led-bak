...
function clearAndInitFqmbpz(type, paramdData) {
	...
	if(ADDTYPE == type) {//增加
		...
		// 赋值文本框和表格加载
		var rowData = $("#datagrid").datagrid("getSelected");
		$("#add_ledid").textbox("setValue", rowData.ledid);
		$("#add_ledmc").textbox("setValue", rowData.ledmc);	
		$("#add_ledbh").textbox("setValue", rowData.ledbh);
		$("#add_jd").textbox("setValue", rowData.jd);
		$("#add_wd").textbox("setValue", rowData.wd);
		$("#add_ledfx").combobox('setValue', rowData.ledfxbm);
		$("#add_qyzt").combobox('setValue', rowData.qyzt);
		$("#change_qyzd").combobox('setValue', rowData.qyzd);
		$("#add_wg").textbox("setValue", rowData.wg);
		$("#add_bdsbipdz").textbox("setValue", rowData.bdsbipdz);
		$("#add_zwym").textbox("setValue", rowData.zwym);
		$("#add_fqmbid").combobox('setValue', rowData.ledmbid);
		$("#change_zdtk").combobox('setValue', rowData.zdtk);
		...
		onLoadSuccess : function() {
			...
			for ( var item in val[0]) {
				if (item == "mbid") {
					...
					//$(this).combobox("select", val[0][item]);
					// 加载完成后,设置选中第四项
					$(this).combobox("select", val[2][item]);
				}
			}
		}
	});
